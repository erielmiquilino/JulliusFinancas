...
export class CardTransactionListComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  openDeleteDialog(transaction: CardTransaction): void {
    ...
    const dialogRef = this.dialog.open(ConfirmDeleteDialogComponent, {
      width: '400px',
      data: {
        entityName: 'o lançamento',
        itemDescription: transaction.description,
        details: [
          {
            label: 'Valor',
            value: currencyPipe.transform(transaction.amount, 'BRL', 'symbol', '1.2-2', 'pt-BR') || ''
          },
          {
            label: 'Data',
            value: datePipe.transform(transaction.date, 'dd/MM/yyyy', 'GMT', 'pt-BR') || ''
          },
          {
            label: 'Parcela',
